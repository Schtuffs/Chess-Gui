...
void BoardTests(void)
{
    TEST("Board::MakeMove: e2e4", [](){
        Board b(DEFAULT_FEN);
        std::string_view expected = "rnbqkbnr/pppppppp/8/8/4P3/8/PPPP1PPP/RNBQKBNR b KQkq e3 0 1";

        Assert::True(b.MakeMove("e2e4"));
        std::string_view actual = b.Fen();
    ...
    TEST("Board::MakeMove: opening London", [](){
        Board b(DEFAULT_FEN);
        std::string_view expected = "r1bqk2r/ppp2ppp/2n1pn2/b2pN3/3P1B2/2P1P3/PP3PPP/RN1QKB1R w KQkq - 1 6";
    ...
    TEST("Board::MakeMove: opening Kings Indian", [](){
        Board b(DEFAULT_FEN);
        std::string_view expected = "rnbq1rk1/ppp2pbp/3p1np1/4p3/2PPP3/2N2N2/PP2BPPP/R1BQK2R w KQ e6 0 6";
    ...
    TEST("Board::MakeMove: simple en passant", [](){
        Board b(DEFAULT_FEN);
        std::string_view expected = "rnbqkbnr/ppp1p1pp/5P2/3p4/8/8/PPPP1PPP/RNBQKBNR b KQkq - 0 3";
