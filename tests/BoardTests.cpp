...
void BoardTests(void)
{
    ...
    TEST("Board::MakeMove: opening Kings Indian", []() {
        Board            b(DEFAULT_FEN);
        std::string_view expected =
            "rnbq1rk1/ppp2pbp/3p1np1/4p3/2PPP3/2N2N2/PP2BPPP/R1BQK2R w KQ e6 0 6";

        std::vector<std::string> moves = {"d2d4", "g8f6", "c2c4", "g7g6", "b1c3", "f8g7",
                                          "e2e4", "d7d6", "g1f3", "O-O",  "f1e2", "e7e5"};
        for (const auto& move : moves) {
            Assert::True(b.MakeMove(move));
        }
        std::string_view actual = b.Fen();
