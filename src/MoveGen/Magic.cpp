...
constexpr u64 GenMask(i32 index, bool isRook)
{
    u64 mask = 0ull;
    i32 rank = index / 8;
    i32 file = index % 8;
...
constexpr u64 SetBlockers(i32 index, i32 maskBits, u64 mask)
{
    u64 blockers = 0;
...
constexpr u64 CalculateAttacks(Index index, u64 pieces, bool isRook)
{
    u64 attacks = 0ull;
...
MagicTable NotCreateMagicTable()
{
    MagicTable table;
    u64        curBishopOffset = 0;
    u64        curRookOffset   = 0;
    ...
    for (Index sq = 0; sq < 64; sq++) {
        ...
        for (int i = 0; i < rookVar; i++) {
            u64 blockers = SetBlockers(i, MAGIC_ROOK_BITS[sq], table.rookMagic[sq].mask);
            u64 hash =
                (u64)(blockers * (u64)table.rookMagic[sq].magic) >> table.rookMagic[sq].shift;
            u64 idx     = table.rookMagic[sq].offset + hash;
            u64 attacks = CalculateAttacks(sq, blockers, true);
            // if (table.rookAttacks[idx] == 0) {
            table.rookAttacks[idx] = attacks;
            // }
        }
...
BitBoard Magic::GetBishopAttacks(Index index, BitBoard blockers)
{
...
BitBoard Magic::GetRookAttacks(Index index, BitBoard blockers)
{
    NotCreateMagicTable();
